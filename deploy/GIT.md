...
### Якщо GitHub пише **«Key is already in use»**

Один публічний ключ не можна прив’язати до двох акаунтів. Зазвичай цей ключ уже доданий (на цьому або іншому акаунті). Варіанти: перевірити **Settings → SSH keys** — якщо ключ уже є, новий не потрібен; або згенерувати **інший** ключ (`ssh-keygen -f ~/.ssh/id_ed25519_moneyflow`) і додати лише його; за потреби — `~/.ssh/config` з `IdentityFile` для `Host github.com`.
